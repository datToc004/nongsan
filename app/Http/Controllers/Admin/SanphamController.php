...
class SanphamController extends Controller
{
    ...
    public function postAdd(SanphamAddRequest $request)
    {
        ...
        $request->file('txtSPImage')->move(
            base_path() . '/resources/upload/sanpham/',
            $filename
        );
        $sanpham = new Sanpham;
        ...
        $sanpham->sanpham_gia = $request->txtPrice;
        ...
        foreach ($files as $file) {
            if (!empty($file)) {
                ...
                $file->move(
                    base_path() . '/resources/upload/chitietsanpham/',
                    $filename
                );
    ...
    public function getDelete($id)
    {
        ...
        $sanpham = Sanpham::findOrFail($id)->donhang;
        if (!$sanpham->isEmpty()) {
            return redirect()->route('admin.sanpham.list')->with(['flash_level' => 'success', 'flash_message' => 'Không thể xóa sản phẩm do đã được đặt hàng!!!']);
        }
